...
def game_state():
    ...
    while not game_failure and not game_quit:
        ...
        if left_pressed and not disable_input: # if key is pressed and if the input keys are not disabled
            ...
            elif left_count >= max(round((1/timing_increase) * 20), 10) and left_count % round(1/(1+(1-timing_increase)/8) * 10) == 0:
        ...
        if right_pressed and not disable_input: # see left
            ...
            if right_count == 1:
                move_right = True
            elif right_count >= max(round((1/timing_increase) * 20), 10) and right_count % round(1/(1+(1-timing_increase)/8) * 10) == 0:
                move_right = True
            else:
                move_right = False
        else:
        ...
        if down_pressed and not disable_input: # see left
            ...
            elif down_count >= max(round((1/timing_increase) * 20), 10) and down_count % round(1/(1+(1-timing_increase)/8) * 10) == 0:
                move_down = True
